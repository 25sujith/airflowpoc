...
def run_spark_job():
    ...
    percentage = spark.sql(
        "SELECT (COUNT(CASE WHEN High > 80 THEN 1 END) / COUNT(*)) * 100 AS percentage_high_above_80 FROM mytable")
    Pearson_Correlation = spark.sql("SELECT corr(High, Volume) AS correlation FROM mytable")
    Max_High_Year = spark.sql(
        """SELECT YEAR(Date) AS Year, MAX(CAST(High AS DOUBLE)) AS max_high FROM mytable GROUP BY Year ORDER BY Year""")
    Avg_Close_For_Each_Month = spark.sql(
        """SELECT YEAR(Date) AS Year, MONTH(Date) AS Month, AVG(Close) AS AvgClose FROM mytable GROUP BY Year, Month ORDER BY Year, Month""")
    ...
    Avg_Close_For_Each_Month.write.csv('/root/airflow/outputfiles/Avg_Close_For_Each_Month.csv', mode='overwrite',
                                       header=True)
